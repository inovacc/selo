...
in **TypeScript, JavaScript, Ruby, Java, and C#**, from the verified Go library — the single
source of truth. Every generated target ships with **golden test vectors** produced by Go and a
runnable test suite, so a wrong port fails its own tests.
...
# Languages: ts | js | ruby | java | csharp     Kinds: all + the 13 document kinds
```
...
task gen:all         # emit all five language targets
# gen:js / gen:ruby / gen:java / gen:csharp and gen:verify:<lang> likewise
```
...
The risk in porting 13 check-digit algorithms into 5 languages is shipping a subtly-wrong
...
`.github/workflows/codegen.yml` runs a matrix over `[ts, js, ruby, java, csharp]` on real
toolchains (node / ruby / JDK+Maven / .NET), executing each language's vector tests. This is the
...
Add the kind to `selo` (it self-registers), then add a `KindPlan` entry in `spec.go`. All five
