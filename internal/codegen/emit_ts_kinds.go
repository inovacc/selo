package codegen

import (
	"fmt"
	"strings"
)

// emit_ts_kinds.go holds the per-kind TypeScript module renderers. Each renders
// a deterministic module from the declarative KindPlan. The check-digit kinds
// reuse the shared mod11.ts reducer; the irregular kinds (CNH coupled DVs, voter
// dual DV, IE-SP rightmost rule, CNS sum-zero, CNPJ char-map, plate/pix regex,
// CEP/phone table lookup) carry bespoke fragments, exactly as the spec Notes flag.

// renderCPF emits the CPF module: two coupled-by-input mod-11 DVs, all-equal
// rejection, mask format, and ninth-digit origin.
func (e tsEmitter) renderCPF(plan KindPlan) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "CPF_REGIONS")

	dv1 := checkDigitLiteral(plan.Checks[0])
	dv2 := checkDigitLiteral(plan.Checks[1])

	fmt.Fprintf(&b, `const DV1: CheckDigit = %s;
const DV2: CheckDigit = %s;

/** validateCPF reports whether value is a valid CPF (formatted or not). */
export function validateCPF(value: string): boolean {
  const d = onlyDigits(value);
  if (d.length !== 11) return false;
  if (allEqual(d)) return false;
  const digits = d.split("").map(Number);
  const dv1 = computeDigit(weightedSum(digits.slice(0, 9), DV1.weights), DV1);
  const dv2 = computeDigit(weightedSum(digits.slice(0, 10), DV2.weights), DV2);
  return dv1 === digits[9] && dv2 === digits[10];
}

/** formatCPF renders value as XXX.XXX.XXX-XX, or throws on bad length. */
export function formatCPF(value: string): string {
  const d = onlyDigits(value);
  if (d.length !== 11) %s
  return `+"`${d.slice(0, 3)}.${d.slice(3, 6)}.${d.slice(6, 9)}-${d.slice(9, 11)}`"+`;
}

/** originCPF returns the issuing region from the 9th digit, or throws. */
export function originCPF(value: string): string {
  const d = onlyDigits(value);
  if (d.length < 9) %s
  const region = CPF_REGIONS[Number(d[8])];
  if (region === undefined) %s
  return region;
}
`, dv1, dv2, formatErrorThrow("ErrInvalidLength"),
		formatErrorThrow("ErrInvalidLength"), formatErrorThrow("ErrInvalidLength"))

	return b.String()
}

// renderSimpleNumeric emits a single-DV numeric kind (PIS): mod-11 DV over the
// first length-1 digits, all-equal rejection, and a mask format.
func (e tsEmitter) renderSimpleNumeric(plan KindPlan, name string, length int) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "")

	dv := checkDigitLiteral(plan.Checks[0])
	base := length - 1
	mask := tsMaskExpr(plan.Mask, "d")

	fmt.Fprintf(&b, `const DV: CheckDigit = %s;

/** validate%[2]s reports whether value is a valid %[2]s. */
export function validate%[2]s(value: string): boolean {
  const d = onlyDigits(value);
  if (d.length !== %[3]d) return false;
  if (allEqual(d)) return false;
  const digits = d.split("").map(Number);
  const dv = computeDigit(weightedSum(digits.slice(0, %[4]d), DV.weights), DV);
  return dv === digits[%[4]d];
}

/** format%[2]s renders the canonical mask, or throws on bad length. */
export function format%[2]s(value: string): string {
  const d = onlyDigits(value);
  if (d.length !== %[3]d) %[5]s
  return %[6]s;
}
`, dv, name, length, base, formatErrorThrow("ErrInvalidLength"), mask)

	return b.String()
}

// renderRenavam emits RENAVAM: single (sum*10)%11 DV, all-equal rejection, and a
// left-pad-to-11 format (no separator mask).
func (e tsEmitter) renderRenavam(plan KindPlan) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "")

	dv := checkDigitLiteral(plan.Checks[0])
	fmt.Fprintf(&b, `const DV: CheckDigit = %s;

/** validateRenavam reports whether value is a valid 11-digit RENAVAM. */
export function validateRenavam(value: string): boolean {
  const d = onlyDigits(value);
  if (d.length !== 11) return false;
  if (allEqual(d)) return false;
  const digits = d.split("").map(Number);
  const dv = computeDigit(weightedSum(digits.slice(0, 10), DV.weights), DV);
  return dv === digits[10];
}

/** formatRenavam left-pads shorter inputs to 11 digits (no separator mask). */
export function formatRenavam(value: string): string {
  let d = onlyDigits(value);
  if (d.length < 11) d = "0".repeat(11 - d.length) + d;
  return d;
}
`, dv)

	return b.String()
}

// renderCNH emits the coupled-DV CNH module (bespoke fragment per the Note):
// DV1 descending 9..1 (raw remainder >=10 -> DV1=0, carry offset 2); DV2
// ascending 1..9 with the offset subtracted before the mod-11 fold.
func (e tsEmitter) renderCNH(_ KindPlan) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "")

	b.WriteString(`/** cnhCheckDigits computes both coupled CNH check digits over the 9-digit base. */
function cnhCheckDigits(base: string): [number, number] {
  let dsc = 0;
  let sum = 0;
  for (let i = 0; i < 9; i++) sum += Number(base[i]) * (9 - i);
  let r = sum % 11;
  let dv1: number;
  if (r >= 10) {
    dv1 = 0;
    dsc = 2;
  } else {
    dv1 = r;
  }
  sum = 0;
  for (let i = 0; i < 9; i++) sum += Number(base[i]) * (1 + i);
  r = (sum % 11) - dsc;
  if (r < 0) r += 11;
  const dv2 = r >= 10 ? 0 : r;
  return [dv1, dv2];
}

/** validateCNH reports whether value is a valid 11-digit CNH. */
export function validateCNH(value: string): boolean {
  const d = onlyDigits(value);
  if (d.length !== 11) return false;
  if (allEqual(d)) return false;
  const [dv1, dv2] = cnhCheckDigits(d.slice(0, 9));
  return dv1 === Number(d[9]) && dv2 === Number(d[10]);
}

/** formatCNH returns the cleaned 11-digit CNH (no separator mask). */
export function formatCNH(value: string): string {
  const d = onlyDigits(value);
  if (d.length !== 11) ` + formatErrorThrow("ErrInvalidLength") + `
  return d;
}
`)

	return b.String()
}

// renderCNS emits the verify-only sum-zero module with prefix constraint.
func (e tsEmitter) renderCNS(plan KindPlan) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "")

	dv := checkDigitLiteral(plan.Checks[0])
	fmt.Fprintf(&b, `const DV: CheckDigit = %s;

/** validateCNS reports whether value is a well-formed CNS (sum %% 11 === 0). */
export function validateCNS(value: string): boolean {
  const d = onlyDigits(value);
  if (d.length !== 15) return false;
  if (allEqual(d)) return false;
  const lead = d[0];
  if (!(lead === "1" || lead === "2" || lead === "7" || lead === "8" || lead === "9")) {
    return false;
  }
  const digits = d.split("").map(Number);
  return computeDigit(weightedSum(digits, DV.weights), DV) === 0;
}

/** formatCNS returns the cleaned 15-digit CNS (no separator mask). */
export function formatCNS(value: string): string {
  const d = onlyDigits(value);
  if (d.length !== 15) %s
  return d;
}
`, dv, formatErrorThrow("ErrInvalidLength"))

	return b.String()
}

// renderCNPJ emits the alphanumeric CNPJ module (bespoke char-map + RL-cycling
// weights per the Note): two DVs, last two chars numeric, all-equal rejection.
func (e tsEmitter) renderCNPJ(plan KindPlan) string {
	var b strings.Builder
	writeHeader(&b, "src/mod11.js", "")

	dv := checkDigitLiteral(plan.Checks[0])
	fmt.Fprintf(&b, `const DV: CheckDigit = %s;

/** cnpjClean uppercases and keeps only [0-9A-Z], capped at 14 chars. */
function cnpjClean(value: string): string {
  let out = "";
  for (const ch of value) {
    const up = ch.toUpperCase();
    if ((up >= "0" && up <= "9") || (up >= "A" && up <= "Z")) {
      out += up;
      if (out.length === 14) break;
    }
  }
  return out;
}

/** cnpjDV computes one check digit over the base string (RL-cycling weights). */
function cnpjDV(base: string): number {
  const vals = base.split("").map(charValue);
  return computeDigit(weightedSum(vals, DV.weights, true), DV);
}

/** validateCNPJ reports whether value is a valid alphanumeric CNPJ. */
export function validateCNPJ(value: string): boolean {
  const c = cnpjClean(value);
  if (c.length !== 14) return false;
  if (allEqual(c)) return false;
  if (c[12] < "0" || c[12] > "9") return false;
  if (c[13] < "0" || c[13] > "9") return false;
  const base = c.slice(0, 12);
  const dv1 = cnpjDV(base);
  const dv2 = cnpjDV(base + String(dv1));
  return dv1 === Number(c[12]) && dv2 === Number(c[13]);
}

/** formatCNPJ renders value as XX.XXX.XXX/XXXX-XX, or throws on bad length. */
export function formatCNPJ(value: string): string {
  const c = cnpjClean(value);
  if (c.length !== 14) %s
  return `+"`${c.slice(0, 2)}.${c.slice(2, 5)}.${c.slice(5, 8)}/${c.slice(8, 12)}-${c.slice(12, 14)}`"+`;
}
`, dv, formatErrorThrow("ErrInvalidLength"))

	return b.String()
}
